...
The backend expects a PostgreSQL database configured through environment variables. See `backend/prisma/schema.prisma` for the current data model.

### Godot Client

Open the `godot/` folder in Godot 4.6 and run the main scene from `project.godot`.

### Dedicated Server

The server scenes and scripts live in `godot/server/`. Deployment support is provided in `deploy/godot-server.Dockerfile` and `deploy/docker-compose.yml`.
